...
type LarkColor string

const (
	ColorBlue      LarkColor = "blue"
	ColorWathet    LarkColor = "wathet"
	ColorTurquoise LarkColor = "turquoise"
	ColorGreen     LarkColor = "green"
	ColorYellow    LarkColor = "yellow"
	ColorOrange    LarkColor = "orange"
	ColorRed       LarkColor = "red"
	ColorCarmine   LarkColor = "carmine"
	ColorViolet    LarkColor = "violet"
	ColorGrey      LarkColor = "grey"
	ColorDefault   LarkColor = "default"
)
...
type LarkMsg struct {
	...
	HeaderColor LarkColor      `json:"-"`              // 标题颜色
}

// FormatMsg 构造一个统计消息卡片
func FormatMsg(f *LarkMsg) *Msg {
	var elements []Element
	md := ""

	mk := maputil.Keys(f.Markdown)
	sort.Strings(mk)
	for _, key := range mk {
		if val, ok := f.Markdown[key]; ok {
			md += fmt.Sprintf("**%s**：%s\n", key, val)
		}
	}
...
// SendLarkMsg 发送消息
func SendLarkMsg(hook string, f *LarkMsg) error {
	if hook == "" {
